...
import { Select, NumberInput, DateInput } from './ui';
import { IconInfo } from './icons';
import type { RegTimeValue, RegTimeRelative, RegTimeAbsolute, DepositValue, ConditionGroupValue, Operator, BoundOp, TimeUnit } from './types';
...
export function RegTimeCondition({ value, onChange, mode = 'relative' }: RegTimeConditionProps) {
  ...
  const update = (patch: Partial<RegTimeRelative> | Partial<RegTimeAbsolute>) =>
    onChange({ ...v, ...patch } as RegTimeValue);
  ...
  return (
    <div style={{
      ...
      {mode === 'absolute' ? (
        <AbsoluteRegTime v={v as RegTimeAbsolute} update={update as (p: Partial<RegTimeAbsolute>) => void} />
      ) : (
        <RelativeRegTime v={v as RegTimeRelative} update={update as (p: Partial<RegTimeRelative>) => void} />
      )}
...
export function formatRegTime(v: RegTimeValue | null | undefined, mode: 'relative' | 'absolute' = 'relative'): string {
  if (!v) return '—';
  const resolvedMode = ('mode' in v ? v.mode : undefined) || mode;
  ...
  const unitText = (u: TimeUnit) => ({ day: '天', week: '周', month: '个月', year: '年' })[u] ?? '';
  if (r.op === 'between') {
...
export function DepositCondition({ value, onChange, rangeMode }: DepositConditionProps) {
  const v: DepositValue = value ?? {
    startDate: '', endDate: '', recentN: '', recentU: 'day',
    op: 'lt', a1: '', a2: '',
  };
...
export function ConditionGroup({ value, onChange, rangeMode, showHintNone = true }: ConditionGroupProps) {
  const v: ConditionGroupValue = value ?? {
