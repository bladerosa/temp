...
| 1.2 | 2026-05-21 | FR-100b 拆细：新增 FR-100b-1（单位自动降级规则：天 / 小时 / 分钟）+ FR-100b-2（X 取系统后台配置，当前 3）+ FR-100b-3（每次刷新页面重算剩余时间）；AC-011a 补充倒计时单位降级与刷新重算两条验收项；F-15 标注图重拍，三行示范单位降级 | PRD Studio Hotfix |
| 1.3 | 2026-05-21 | v1.2 自审：把 FR-045a 与 §3.3 用户流程图中残留的 literal「3 天后」改写为「N {天\|小时\|分钟} 后」泛化表达，并标注「刚提交时为 3 天后」作为示例锚点，避免与 FR-100b-1 单位降级规则冲突 | PRD Studio Hotfix |
...
flowchart TD
    ...
    I --> I1[订单仍在「待供应商付款」Tab；<br/>行内「确认付款」按钮隐藏，<br/>详情按钮左侧出现状态文字<br/>「等待用户确认，或 N 天 / 小时 / 分钟后自动确认」<br/>（单位随剩余时间降级，详见 FR-100b-1）<br/>（供应商退款 按钮仍可用）]
...
| FR-045a | 在本 Tab 内已经走完「确认付款」流程的行进入过渡态，行操作区不再显示「确认付款」按钮，并在「详情」按钮左侧显示状态文字「等待用户确认，或 N {天\|小时\|分钟} 后自动确认」（单位按剩余时间自动降级，刚提交时为「3 天后自动确认」）；详细行为定义见 §7.10.1（FR-100a 至 FR-100d）。过渡态外观参见 F-15（`workflow/figures/F15-paying-paid-pending.png`）。 |
...
![F-15 待供应商付款 / 三种倒计时单位形态对照](workflow/figures/F15-paying-paid-pending.png)

图中编号说明（前三行示范倒计时单位「天 / 小时 / 分钟」自动降级）：
- ① 第 1 行：刚刚提交「确认付款」，剩余约 3 天 → 状态文字「等待用户确认，或 3 天后自动确认」
- ② 第 2 行：已提交「确认付款」约 2.5 天，剩余 < 1 天 → 自动降级到小时 → 状态文字「等待用户确认，或 12 小时后自动确认」
- ③ 第 3 行：已提交「确认付款」接近上限，剩余 < 1 小时 → 自动降级到分钟 → 状态文字「等待用户确认，或 5 分钟后自动确认」
- ④ 第 4 行（参照对照）尚未提交「确认付款」，仍正常显示 详情 / 确认付款 / 供应商退款 三个按钮——表明同表内不同订单状态互相不影响
...
| FR-100b | 进入过渡态后，该行「详情」按钮左侧出现一行状态文字「等待用户确认，或 N {天\|小时\|分钟} 后自动确认」（小灰字、不可点击、不允许换行）。单位按本表 FR-100b-1 的「自动降级」规则切换；N 的取值见 FR-100b-2。 |
| FR-100b-1 | **单位自动降级规则**：剩余时间 ≥ 1 天 → 显示「天」单位；剩余时间 < 1 天且 ≥ 1 小时 → 显示「小时」单位；剩余时间 < 1 小时 → 显示「分钟」单位（下限为 1，避免出现「0 分钟」）。三种形态举例：「等待用户确认，或 3 天后自动确认」/「等待用户确认，或 12 小时后自动确认」/「等待用户确认，或 5 分钟后自动确认」（见 F-15 ①②③）。 |
| FR-100b-2 | **超时阈值 X 取系统后台配置**：自动确认的截止时间 = 操作员提交「确认付款」的时刻 + X 天。当前 X = 3（与系统后台写死的配置保持一致）。如未来后台配置变更，本字段统一刷新——本界面不单独维护此常量。 |
| FR-100b-3 | **每次进入或刷新页面时重算剩余时间**：状态文字按「截止时间 − 当前时刻」即时计算，刷新页面或切换 Tab 重新进入时，单位与数字均跟随重算结果而变化。无需周期性自动刷新（不实现客户端实时倒计时滚动）。 |
...
- [ ] 该行「详情」按钮左侧出现一行状态文字，**刚提交时**文案为「等待用户确认，或 3 天后自动确认」（X = 3 取系统后台配置；如后台配置变更则跟随）；样式为小灰字、不可点击、不允许换行
- [ ] 倒计时单位自动降级验证：
  - 剩余时间 ≥ 1 天 → 状态文字以「天」为单位（例：「等待用户确认，或 3 天后自动确认」）
  - 剩余时间 < 1 天且 ≥ 1 小时 → 状态文字以「小时」为单位（例：「等待用户确认，或 12 小时后自动确认」）
  - 剩余时间 < 1 小时 → 状态文字以「分钟」为单位（例：「等待用户确认，或 5 分钟后自动确认」；下限 1，不出现「0 分钟」）
- [ ] 刷新页面（或切换离开后再回到「待供应商付款」Tab）时，状态文字会按当前时刻重新计算剩余时间——单位与数字均跟随重新计算结果而变化
- [ ] 提交后订单仍停留在「待供应商付款」Tab，**不**进入「已完成」Tab
- [ ] 该行的「供应商退款」按钮在过渡态期间仍可点击，点击后正常打开供应商退款弹窗（M-06）
- [ ] 同表内不同行互不影响：已提交「确认付款」的行显示状态文字且隐藏「确认付款」按钮；同表内未提交「确认付款」的行仍正常显示「详情 / 确认付款 / 供应商退款」三个按钮
