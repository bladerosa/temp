// TargetSelector: used in normal notification; lets user pick "by options" or "by conditions"
// by options: dropdown (preserves existing behaviour)
// by conditions: uses ConditionGroup (rangeMode=absolute) + Submit query → result card
...
function TargetSelector({ value, onChange }) {
  const v = value || {
    method: 'option',    // 'option' | 'condition'
    optionValue: 'all',
    conditionValue: null,
    queryResult: null,   // { count, sampleIds, queriedAt }
  };
  ...
  const handleClear = () => {
    update({ conditionValue: null, queryResult: null });
  };
  ...
  return (
    <div>
      {/* Method selector */}
      <div style={{ display: 'flex', gap: 0, marginBottom: 14, background: '#F2F3F7', padding: 3, borderRadius: 9, width: 'fit-content' }}>
        {[
          { key: 'option', label: '按选项', desc: '从固定分群中选择' },
          { key: 'condition', label: '按条件', desc: '自定义筛选规则' },
        ].map(t => {
      ...
      {v.method === 'option' ? (
        ...
        <div>
          ...
          {v.queryResult && (
            <QueryResultCard result={v.queryResult} />
          )}
        </div>
      )}
    </div>
...
function QueryResultCard({ result }) {
  return (
    <div style={{
      ...
      <Button variant="secondary" size="sm" icon={<IconDownload size={14} />} onClick={() => alert('下载用户信息表（示意）')}>
        下载用户表
      </Button>
    </div>
  );
...
Object.assign(window, { TargetSelector });
